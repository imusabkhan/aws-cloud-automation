...
SLACK_WEBHOOK_URL = 'https://hooks.slack.com/'  # Replace with your webhook URL # Replace with your webhook URL
...
def format_results_for_slack(results):
    """Format the IAM permission results for Slack."""
    if not results:
        return "No sensitive permissions detected."

    formatted_message = "Detected Sensitive Permissions:\n"
    for result in results:
        formatted_message += f"*Account*: {result['account_name']}\n"
        # formatted_message += f"*Resource Type*: {result['resource_type']}\n"
        formatted_message += f"*{result['resource_type']}*: {result['resource_name']}\n"
        if result['privilege_escalation']:
            formatted_message += f"  - *Privilege Escalation Permissions*: {', '.join(result['privilege_escalation'])}\n"
        if result['destructive_actions']:
            formatted_message += f"  - *Destructive Actions*: {', '.join(result['destructive_actions'])}\n"
        formatted_message += "\n"
...
if __name__ == "__main__":
    ...
    if sensitive_permissions_results:
        ...
        slack_message = format_results_for_slack(sensitive_permissions_results)
        send_to_slack(slack_message)    
    else:
        print(f"No sensitive permissions found for account {account_name} ({account_id}).\n")
    print("\nSensitive permissions check complete.")
    
    # results = check_user_roles_and_policies(account_name, account_id)
